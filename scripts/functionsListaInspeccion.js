...
btnGenerar.addEventListener("click", async function generarPDF(e) {
  ...
  let evaluarDatosPrincipales = () => {
    ...
    if (trabajo != "" && lugar != "" && responsableNombre != "Seleccionar" && supervisorNombre != "Seleccionar") {
      doc.text(trabajo, 42, 21);
      doc.text(lugar, 31.8, 23.2);
      doc.text(fecha, 31.8, 25.4);
      doc.text(hora, 47, 25.4);
      doc.text("Bernabe Oscco León", 136, 24);

      //tecnico responsable
      doc.text(responsableNombre, 90, 286 ,{align: "center"})
      doc.addImage(responsableFirma, "PNG", 76, 277.5, 24, 6);

      //supervisor
      doc.text(supervisorNombre, 174, 286, {align: "center"});
      doc.addImage(supervisorFirma, "PNG", 162, 277.5, 24, 6);
  ...
  doc.text(comentarios, 24.5, 272.4, {
    maxWidth: 164,
  ...
  function evaluarNombreCargo() {
    ...
    cargos.forEach((cargo) => {
      if (cargo.value != "") {
        if (contador != 0) {
          doc.text(cargo.value, cargosX, 29.4, {
            align: "center",
          });
          cargosX += 32;
        } else {
          doc.text(cargo.value, 83, 29.4, {
            align: "center",
          });
          contador = 1;
        }
    ...
    nombres.forEach((nombre) => {
      if (nombre.value != "") {
        if (contadorn != 0) {
          doc.setFontSize(4)
          doc.text(nombre.value, nombresX1, 31.5, {
            align: "center",
            maxWidth: 20,
            lineHeightFactor: 1,
          });
  ...
  eppEspecial.forEach((e) => {
    ...
    if(contadorColumnas==1){
      positionSelectX += 31.5;
    }else{
      positionSelectX+=31.3
    }
    contadorElementos=1
    contadorColumnas+=1
    positionSelectY = 39;
  ...
  contadorColumnas = 1
  positionSelectX = 67.6;
  positionSelectY = 134.4;
